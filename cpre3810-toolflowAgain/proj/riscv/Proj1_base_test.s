...
# --- Shifts with register shift amount (rs2 low 5 bits) ---
sll   x8,  x7, x1            # x8  = 8 << 5
srl   x9,  x7, x1            # x9  = 8 >> 5 (logical)
sra   x10, x2, x1            # x10 = -3 >> 5 (arithmetic)
...
# --- Set-less-than (signed/unsigned), reg and imm ---
slt   x18, x2,  x1           # x18 = (-3 < 5)  ? 1 : 0  (signed)
sltu  x19, x2,  x1           # x19 = (0xFFFFFFFD < 5) ? 0 : 0 (unsigned)
slti  x20, x2, -1            # x20 = (-3 < -1) ? 1 : 0
sltiu x21, x2, -1            # unsigned compare with imm = 0xFFF (sign-ext)

# --- Upper-immediate and PC-relative ALU ---
lui   x22, 0x12345           # x22 = 0x12345000
addi  x22, x22, 0x678        # x22 = 0x12345678  (12-bit imm)
auipc x23, 0                 # x23 = PC
add   x24, x23, x1           # consume AUIPC result
